...
typedef struct
{
    bool odd, follow_both, follow_free;
    node n;
} bfs_info;
					     ...
					     const int phase )
{
	...
	forall_inout_edges( e, i.n ) {
		...
		nexti.follow_both = false;
		nexti.follow_free = false;
		Q.append( nexti );
...
static void prune_edges( graph & G,
			 ...
			 const edge_array < int >&rank,
			 node_array < bool > &reached,
			 const node_array < bool > &odd, int phase )
{
    edge e;
    node v, w;
...
static bool bfs_HK( graph & G,
		    ...
		    const edge_array < int >&rank, const int phase )
{
    list < bfs_info > Q;
    node v, w;
    edge e;
	...
	forall_inout_edges( e, v ) {
		...
		if ( !augmenting_path_found ) {
		    ...
		    nf.n = w;
		    nf.follow_free = !f.follow_free;
		    Q.append( nf );
		}
	    }
	    if ( dist[w] == dv + 1 )
		useful[e] = phase_number_HK;
	}
    }
...
static d_edge find_aug_path_HK( graph & G,
				...
				const edge_array < int >&rank,
				const int phase, bool f_matched )
{
    node w = f.t;
	...
	if ( ( !f_matched && is_matched( e ) )
	     || ( f_matched && !is_matched( e ) ) )
	    continue;

	d_edge fol;

	fol.e = e;
	fol.t = G.opposite( e, w );
	fol.s = w;
	if ( pred[fol.t].e != nil || useful[e] != phase_number_HK )
	    continue;
					    ...
					    const int phase )
{
	...
	forall( v, free_in_A ) {
		...
		if ( pred[G.opposite( e, v )].e == nil
		     && useful[e] == phase_number_HK ) {
		    d_edge fol;

		    fol.e = e;
		    fol.s = v;
		    fol.t = G.opposite( v, e );
		    d_edge f = find_aug_path_HK( G, fol, free, mate,
						 pred, useful, rank, phase,
						 true );
		    if ( f.e ) {
			EL.append( f );
			break;
...
list < edge > BI_RANK_MAX_MATCHING( graph & G, const edge_array < int >&rank )
{
	...
	if ( rank[e] <= 0 )
	    leda::error_handler( 999,
				 "RANK_MAX: illegal rank (non-positive)" );
#endif
...
be between 1 and n" );
	...
	return leda::MAX_WEIGHT_BIPARTITE_MATCHING_T < integer > ( G, A, B,
								   w );
}
